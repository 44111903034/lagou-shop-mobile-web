<template>
    <van-pull-refresh class="home-main" v-model="refreshing" @refresh="onRefresh">
      ...
      <van-grid icon-size="40">
        <van-grid-item :icon="item.img" :text="item.info[0].value" v-for="(item,index) in menusData" :key="index"/>
      </van-grid>
      <!-- 区域3: 公告栏 -->
...
const initProductsData = async () => {
  ...
  if (data.status !== 200) { return }
  // 请求每次得到的是新一段的数据，需要将新数据添加到 productsData 中
