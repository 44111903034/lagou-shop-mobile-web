    ...
    <van-tabs scrollspy color="#fbc546">
        ...
        <van-popup v-model:show="specState.show" position="bottom" class="popup">
          <van-cell-group>
           ...
           <van-cell class="spec" v-for="(attr, specIndex) in productAttr" :key="attr.id">
            ...
            <!-- 规格选择标签 -->
            <span class="tag" v-for="tag in attr.attr_values" :key="tag" v-text="tag" :class="{ active: specState.spec[specIndex] === tag}" @click="handleTagChange(tag, specIndex)"></span>
           </van-cell>
           <!-- 数量 -->
           <van-cell title="数量">
      ...
      <van-tab title="评价" class="comment">
        <van-cell-group>
          ...
          <!-- 评价列表 -->
          <comment-item :reply="reply" v-if="replyCount !== 0" />
        </van-cell-group>
      </van-tab>
      <van-tab title="推荐">
